...
class AppState:
    ...
    def team_strategy(self) -> str:

        """Return the active Journey's saved Team Strategy."""

        if self.journey is None:

            return DEFAULT_TEAM_STRATEGY

        strategy = self.journey.get(
        ...
        if (
            ...
            and strategy in VALID_TEAM_STRATEGIES
    ...
    async def save_team_strategy(
        ...
        """Persist the Journey's Team Strategy."""

        if self.journey is None:

            return False

        if strategy not in VALID_TEAM_STRATEGIES:

            raise ValueError(

                f"Unsupported Team Strategy: {strategy}"

            )

        had_previous = "team_strategy" in self.journey

        previous_strategy = self.journey.get("team_strategy")

        self.journey["team_strategy"] = strategy
        ...
        try:
            ...
            if had_previous:
                ...
                self.journey["team_strategy"] = previous_strategy

            else:

                self.journey.pop("team_strategy", None)
